...
df1 = spark.sql("""
    ...
    FindFirstFutures AS (
        SELECT v.mongo_hostname, MAX(v.Since) AS InitialFutureDate
        FROM op_count_item AS v
        JOIN partition_ranges AS P
            ON p.is_first_partition
            AND p.mongo_hostname = v.mongo_hostname
            AND p.days_post_epoch = v.days_post_epoch
            AND p.seconds_oF_day = v.seconds_of_day
        GROUP BY v.mongo_hostname, v.days_post_epoch, v.seconds_of_day
    ),
    AllBeforeAndAfter AS (
        SELECT p.*,
            ...
            f.InitialFutureDate < v.IsKnownAsOf AS IsFutureCollection
        FROM AllKnownCollectionsEver AS v
            JOIN partition_ranges AS p
                ON v.mongo_hostname = p.mongo_hostname
            JOIN FindFirstFutures AS f
                ON v.mongo_hostname = f.mongo_hostname
                AND p.mongo_hostname = f.mongo_hostname
...
df1.show(775000, truncate=False)
